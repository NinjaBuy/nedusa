...
import { SearchUtils } from "@ninjajs/utils"
  ...
  constructor(_, options: MeilisearchPluginOptions) {
    ...
    if (process.env.NODE_ENV !== "development") {
      if (!options.config?.apiKey) {
        throw Error(
          "Meilisearch API key is missing in plugin config. See https://docs.ninjajs.com/add-plugins/meilisearch"
        )
      }
    }
    ...
    if (!options.config?.host) {
      throw Error(
        "Meilisearch host is missing in plugin config. See https://docs.ninjajs.com/add-plugins/meilisearch"
      )
    }
